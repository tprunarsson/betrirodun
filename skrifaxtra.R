...
# lets quickly analyse who on the list got an OR
listsize = nrow(Bidlisti)
hist_completed = rep(FALSE,listsize)
hist_date = rep(ymd(NA),listsize)
hist_room = rep("",listsize)
hist_laeknir = rep("",listsize)
hist_kennitala = rep("",listsize)
hist_kort = rep("",listsize)
hist_sergrein = rep("",listsize)
hist_acute = rep("", listsize)
hist_a_bidlista = rep(ymd(NA),listsize)
for (i in seq(1,listsize,1)) {
  ...
  if (kt %in% O$Kennitala) {
    ...
    if (length(idx) > 1)
      stop()
    if (length(ix) > 0) {
      hist_completed[i] = TRUE
      hist_date[i] = convertToDate(O$Date.of.operation[idx])
      hist_room[i] = O$`Aðgerða-stofa`[idx]
      hist_laeknir[i] = O$Aðalskurðlæknir[idx]
      hist_kennitala[i] = O$Kennitala[idx]
      hist_kort[i] = O$Aðgerðarkort[idx]
  ...
  #        print(c(hist_a_bidlista[i],convertToDate(B$Skráð.á.biðlista.Dagur[idk])))
      ...
      #print(c(kt,Bidlisti$OrbitOperation.OperationCard[i],hist_date[i],hist_room[i]))
    }
  }
}
...
cat("set ignoreCode := \n", file=fname, sep=" ", append=TRUE)
for(i in c(1:nrow(Bidlisti))) {
  if ((Bidlisti$Merki[i] == 4) & (hist_completed[i] == FALSE))  {
...
# Aðgerðir sem vorum framkvæmdar (ný gögn)
cat("param performedSurgery := \n", file=fname, sep=" ", append=TRUE)
for(i in c(1:nrow(Bidlisti))){
...
idx <- which(hist_completed & (Bidlisti$Merki == 4))
idx3 <- which(hist_completed & (hist_room == "Hb. Stofa 3") & (Bidlisti$Merki != 4))
idx6 <- which(hist_completed & (hist_room == "Hb. Stofa 6") & (Bidlisti$Merki != 4))

if (length(idx6) > 0) {
  print("Stofa 6 auka:")
  print(hist_sergrein[idx6])
  print(hist_laeknir[idx6])
}
if (length(idx3) > 0) {
  print("Stofa 3 auka:")
  print(hist_sergrein[idx3])
